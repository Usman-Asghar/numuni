...
<table>
        <tr>
			...
			<tr>
            <td><?php echo $this->lang->line('Quote') ?></td><td><?php  echo  prefix(1). $invoice['tid'] ?></td>
			</tr>
			<tr>
            <td><?php echo $this->lang->line('Quote Date') ?></td><td><?php echo $invoice['invoicedate'] ?></td>
			</tr>
			<tr>
            <td><?php echo $this->lang->line('Valid till') ?></td><td><?php echo $invoice['invoiceduedate'] ?></td>
			</tr>
    ...
    <table class="plist" cellpadding="0" cellspacing="0">
        ...
        foreach ($products as $row) {
            ...
            if ($invoice['tax'] > 0) { $cols++; echo '<td style="width:16%;">' . amountExchange($row['totaltax'],$invoice['multi']) . ' <span class="tax">(' . amountExchange($row['tax'],$invoice['multi']) . '%)</span></td>'; }
 ...
 if ($invoice['shipping'] > 0) { $cols++;}
    ...
    <table class="subtotal">
        ...
        <tbody>
        <tr>
            <td class="myco2" rowspan="<?php echo $cols ?>"><?php echo '<strong>'.$this->lang->line('Status').': ' . $this->lang->line(ucwords($invoice['status'])) ?></strong><hr><?php echo ''.$this->lang->line('Proposal').': </br></br><small>' . $invoice['proposal'] ?></small>
                </td>
        ...
        <tr>
            ...
            <td><?php echo amountExchange($sub_t); ?></td>
        </tr>
